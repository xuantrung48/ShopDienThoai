    ...
    var wordcount = (function() {
        ...
        var isWordBoundary = function(map, index) {
            ...
            if (nextType === ci.EXTENDNUMLET &&
                (type === ci.ALETTER || type === ci.NUMERIC || type === ci.KATAKANA || type === ci.EXTENDNUMLET)) {
                return false;
            }
        ...
        var doGetWords = function(str, options) {
            ...
            for (; i < len; ++i) {
                ...
                if (WordBoundary.isWordBoundary(map, i)) {
                    ...
                    if (word &&
                        (includeWhitespace || !WHITESPACE$1.test(word)) &&
                        (includePunctuation || !PUNCTUATION$1.test(word))) {
                        if (isProtocol(word)) {
        ...
        var getText = function(node, schema) {
            ...
            var isSeparator = function(node) {
                return blockElements[node.nodeName] ||
                    shortEndedElements[node.nodeName] ||
                    whiteSpaceElements[node.nodeName];
            };
            var txt = "";
