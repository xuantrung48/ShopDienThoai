    ...
    var importcss = (function() {
        ...
        var getSelectors = function(editor, doc, fileFilter) {
            ...
            function append(styleSheet, imported) {
                ...
                global$4.each(styleSheet.imports,
                    function(styleSheet) {
                ...
                global$4.each(rules,
                    function(cssRule) {
                        if (cssRule.styleSheet) {
                            ...
                            global$4.each(cssRule.selectorText.split(","),
                                function(selector) {
            ...
            global$4.each(editor.contentCSS,
                function(url) {
        ...
        var getGroupsBySelector = function(groups, selector) {
            return global$4.grep(groups,
                function(group) {
                    return !group.filter || group.filter(selector);
                });
        };
        var compileUserDefinedGroups = function(groups) {
            return global$4.map(groups,
                function(group) {
        ...
        var setup = function(editor) {
            editor.on("renderFormatsMenu",
                function(e) {
                    ...
                    global$4.each(
                        getSelectors(editor, e.doc || editor.getDoc(), compileFilter(Settings.getFileFilter(editor))),
                        function(selector) {
                            if (selector.indexOf(".mce-") === -1) {
                                if (!selectorFilter || selectorFilter(selector)) {
                                    var selectorGroups = getGroupsBySelector(groups, selector);
                                    if (selectorGroups.length > 0) {
                                        global$4.each(selectorGroups,
                                            function(group) {
