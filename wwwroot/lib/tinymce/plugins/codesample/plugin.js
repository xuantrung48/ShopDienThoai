(function() {
    var codesample = (function(domGlobals) {
        ...
        var _self = typeof window !== "undefined"
            ? window
            : typeof WorkerGlobalScope !== "undefined" && domGlobals.self instanceof WorkerGlobalScope
            ? domGlobals.self
            : {};
        var Prism = function() {
            ...
            var _ = _self.Prism = {
                ...
                highlightAll: function(async, callback) {
                    var elements = domGlobals.document.querySelectorAll(
        ...
        if (Prism.languages.markup) {
            Prism.languages.insertBefore("markup",
                "tag",
                {
                    style: {
        ...
        Prism.languages.insertBefore("c",
            "string",
            {
                macro: {
                    pattern: /(^\s*)#\s*[a-z]+([^\r\n\\]|\\.|\\(?:\r\n?|\n))*/im,
        ...
        Prism.languages.insertBefore("csharp",
            "keyword",
            {
                preprocessor: {
                    pattern: /(^\s*)#.*/m,
        ...
        Prism.languages.insertBefore("cpp",
            "keyword",
            {
                'class-name': {
            ...
            Prism.hooks.add("wrap",
                function(env) {
                    if (env.language === "php" && env.type === "markup") {
                        env.content =
                            env.content.replace(/(\{\{\{PHP[0-9]+\}\}\})/g, '<span class="token php">$1</span>');
                    }
                });
            Prism.languages.insertBefore("php",
                "comment",
        ...
        (function(Prism) {
            ...
            Prism.languages.insertBefore("ruby",
                "keyword",
                {
                    regex: [
                        {
        ...
        var setup = function(editor) {
            ...
            editor.on("PreProcess",
                function(e) {
                    $("pre[contenteditable=false]", e.node).filter(Utils.trimArg(Utils.isCodeSample)).each(
                        function(idx, elm) {
            ...
            editor.on("SetContent",
                function() {
                    var unprocessedCodeSamples = $("pre").filter(Utils.trimArg(Utils.isCodeSample)).filter(
                        function(idx, elm) {
