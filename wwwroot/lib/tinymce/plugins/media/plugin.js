    ...
    var media = (function() {
        ...
        var getData = function(editor) {
            ...
            return element.getAttribute("data-mce-object")
                ? HtmlToData.htmlToData(Settings.getScripts(editor),
            ...
            var generalFormItems = [
                {
                    ...
                    onpaste: function() {
                        setTimeout(function() {
                                Service.getEmbedHtml(editor, win.toJSON()).then(addEmbedHtml(win, editor))
                                    .catch(handleError(editor));
                            },
                            1);
                    },
        ...
        var setup = function(editor) {
            editor.on("preInit",
                function() {
                    ...
                    global$2.each("video audio iframe object".split(" "),
                        function(name) {
                            specialElements[name] = new RegExp("</" + name + "[^>]*>", "gi");
                        });
                    var boolAttrs = editor.schema.getBoolAttrs();
                    global$2.each("webkitallowfullscreen mozallowfullscreen allowfullscreen".split(" "),
                        function(name) {
                            boolAttrs[name] = {};
                        });
                    editor.parser.addNodeFilter("iframe,video,audio,object,embed,script",
                        Nodes.placeHolderConverter(editor));
                    editor.serializer.addAttributeFilter("data-mce-object",
                        function(nodes, name) {
                            var i = nodes.length;
                            var node;
                            var realElm;
            ...
            editor.on("objectResized",
                function(e) {
                    ...
                    if (target.getAttribute("data-mce-object")) {
                        ...
                        if (html) {
                            html = unescape(html);
                            target.setAttribute("data-mce-html",
                                escape(UpdateHtml.updateHtml(html,
                                    {
                                        width: e.width,
                                        height: e.height
                                    })));
