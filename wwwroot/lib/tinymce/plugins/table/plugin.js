(function() {
    var table = (function(domGlobals) {
        ...
        var Immutable = function() {
            ...
            return function() {
                ...
                if (fields.length !== values.length) {
                    throw new Error('Wrong number of arguments to struct. Expected "[' +
                        fields.length +
                        ']", got ' +
                        values.length +
                        " arguments");
        ...
        var invalidTypeMessage = function(incorrect, type) {
            throw new Error("All values need to be of type: " +
                type +
                ". Keys (" +
                sort(incorrect).join(", ") +
                ") were not.");
        ...
        var checkDupes = function(everything) {
            ...
            var dupe = find(sorted,
                function(s, i) {
                    return i < sorted.length - 1 && s === sorted[i + 1];
                });
            dupe.each(function(d) {
                throw new Error("The field: " +
                    d +
        ...
        var MixedBag = function(required, optional) {
            ...
            return function(obj) {
                ...
                each(optional,
                    function(opt) {
                        r[opt] = constant(Object.prototype.hasOwnProperty.call(obj, opt)
                            ? Option.some(obj[opt])
                            : Option.none());
                    });
                return r;
        ...
        var clone = function(element) {
            return foldl(element.dom().attributes,
                function(acc, attr) {
        ...
        var internalSet = function(dom, property, value) {
            if (!isString(value)) {
                domGlobals.console.error("Invalid call to CSS.set. Property ",
                    property,
                    ":: Value ",
                    value,
                    ":: Element ",
        ...
        var browsers = [
            {
                ...
                search: function(uastring) {
                    return contains$1(uastring, "edge/") &&
            ...
            },
            {
                ...
                search: function(uastring) {
                    return (contains$1(uastring, "safari") || contains$1(uastring, "mobile/")) &&
                        contains$1(uastring, "applewebkit");
                }
            }
        ];
            ...
            return map(rows,
                function(row) {
                    ...
                    var parentSection = parent$1.map(function(p) {
                        ...
                        return parentName === "tfoot" || parentName === "thead" || parentName === "tbody"
                            ? parentName
                            : "tbody";
                    }).getOr("tbody");
                    var cells = map(TableLookup.cells(row),
                        function(cell) {
        ...
        var findItem = function(warehouse, item, comparator) {
            var filtered = filterItems(warehouse,
                function(detail) {
        ...
        var clean = function(table, stats) {
            var emptyRows = filter(LayerSelector.firstLayer(table, "tr"),
                function(row) {
                    return row.dom().childElementCount === 0;
                });
            each(emptyRows, remove$2);
            if (stats.minCol() === stats.maxCol() || stats.minRow() === stats.maxRow()) {
                each(LayerSelector.firstLayer(table, "th,td"),
                    function(cell) {
                        remove(cell, "rowspan");
                        remove(cell, "colspan");
                    });
        ...
        var extract = function(table, selectedSelector) {
            ...
            var unselectedCells = LayerSelector.filterFirstLayer(table,
                "th,td",
                function(cell) {
        ...
        var getEnd = function(element) {
            return name(element) === "img"
                ? 1
                : getOption(element).fold(function() {
        ...
        var cloneFormats = function(oldCell, newCell, formats) {
            ...
            return first$1.map(function(firstText) {
                ...
                var parents = ancestors$1(firstText,
                    formatSelector,
                    function(element) {
                        return eq(element, oldCell);
                    });
                return foldr(parents,
        ...
        var inSelection = function(bounds, detail) {
            ...
            return leftEdge <= bounds.finishCol() &&
        ...
        var getBounds = function(detailA, detailB) {
            return bounds(Math.min(detailA.row(), detailB.row()),
        ...
        var sharedOne$1 = function(look, elements) {
            return Parent.sharedOne(universe,
        ...
        var breakPath$2 = function(child, isTop, breaker) {
            return Parent.breakPath(universe,
                child,
                isTop,
                function(u, p, c) {
                    return breaker(p, c);
                ...
                return lookupTable(start).bind(function(startTable) {
                    return lookupTable(finish).bind(function(finishTable) {
                        if (eq(startTable, finishTable)) {
                            ...
                            var finishCell = ancestorCells.length > 0
                                ? ancestorCells[ancestorCells.length - 1]
                                : finish;
                            return Option.some(Identified.create({
                                boxes: TablePositions.nestedIntercepts(startTable,
                                    start,
                                    startTable,
                                    finish,
                                    finishTable),
                            ...
                            return Option.some(Identified.create({
                                boxes: TablePositions.nestedIntercepts(finishTable,
                                    start,
                                    startTable,
                                    finish,
                                    finishTable),
            ...
            return CellSelection.getEdges(container, firstSelectedSelector, lastSelectedSelector).bind(function(edges) {
                ...
                return firstAncestor.bind(function(fA) {
                    return lastAncestor.bind(function(lA) {
                        return eq(fA, lA)
                            ? TablePositions.getBox(edges.table(), edges.first(), edges.last())
                            : Option.none();
                    });
                });
            });
                    ...
                    adt[key] = function() {
                        ...
                        var match = function(branches) {
                            ...
                            if (constructors.length !== branchKeys.length) {
                                throw new Error("Wrong number of arguments to match. Expected: " +
                                    constructors.join(",") +
                                    "\nActual: " +
                                    branchKeys.join(","));
                            }
                            var allReqd = forall(constructors,
                                function(reqKey) {
                                    return contains(branchKeys, reqKey);
                                });
                            if (!allReqd) {
                                throw new Error("Not all branches were specified when using match. Specified: " +
                                    branchKeys.join(", ") +
                                    "\nRequired: " +
                                    constructors.join(", "));
                            }
                        ...
                        return {
                            fold: function() {
                                if (arguments.length !== cases.length) {
                                    throw new Error("Wrong number of arguments to fold. Expected " +
                                        cases.length +
                                        ", got " +
                                        arguments.length);
                                }
        ...
        var unmergable = function(cell, selections) {
            var hasSpan = function(elem) {
                return has(elem, "rowspan") && parseInt(get(elem, "rowspan"), 10) > 1 ||
                    has(elem, "colspan") && parseInt(get(elem, "colspan"), 10) > 1;
            };
            ...
            return SelectionTypes.cata(selections.get(),
                ...
                function(cells, _env) {
                    ...
                    return TableSelection
                        .retrieveBox(table, Ephemera.firstSelectedSelector(), Ephemera.lastSelectedSelector()).bind(
                            function(bounds) {
                    ...
                    if (e.selection === true && e.paste === true) {
                        ...
                        cellOpt.each(function(domCell) {
                            ...
                            TableLookup.table(cell).each(function(table) {
                                var elements = filter(fromHtml$1(e.content),
                                    function(content) {
        ...
        var findPositions = function(getInnerEdge, getOuterEdge, array) {
            ...
            var lines = map(array.slice(1),
                function(cellOption, index) {
        ...
        var remove$3 = function(element, attr, id) {
            var nu = filter(read(element, attr),
                function(v) {
                    return v !== id;
                });
            if (nu.length > 0) {
        ...
        var deriveRows = function(rendered, generators) {
            var findRow = function(details) {
                var rowOfCells = findMap(details,
                    function(detail) {
        ...
        var findInWarehouse = function(warehouse, element) {
            var all = flatten(map(warehouse.all(),
                function(r) {
                    return r.cells();
                }));
            return find(all,
        ...
        var onPasteRows = function(warehouse, target) {
            var details = map(target.selection(),
                function(cell) {
        ...
        var measure = function(startAddress, gridA, gridB) {
            if (startAddress.row() >= gridA.length || startAddress.column() > GridRow.cellLength(gridA[0])) {
                return Result.error("invalid start address out of table bounds, row: " +
                    startAddress.row() +
                    ", column: " +
                    startAddress.column());
            }
        ...
        var rowFill = function(grid, amount, generator) {
            return grid.concat(repeat(amount,
                function(_row) {
        ...
        var isSpanning = function(grid, row, col, comparator) {
            ...
            return grid.length > 1 &&
                GridRow.cellLength(currentRow) > 1 &&
            ...
            for (var r = startRow; r < endRow; r++) {
                for (var c = startCol; c < endCol; c++) {
                    if (isSpanning(gridA, r, c, comparator)) {
                        MergingOperations.unmerge(gridA,
        ...
        var insert = function(index, gridA, gridB, generator, comparator) {
            ...
            return fittedOldGrid.slice(0, index).concat(fittedNewGrid)
        ...
        var insertRowAt = function(grid, index, example, comparator, substitution) {
            ...
            var between = GridRow.mapCells(grid[example],
                function(ex, c) {
                    var withinSpan = index > 0 &&
                        index < grid.length &&
                        comparator(GridRow.getCellElement(grid[index - 1], c), GridRow.getCellElement(grid[index], c));
                    var ret = withinSpan
            ...
            return map(grid,
                function(row) {
                    ...
                    var sub = withinSpan
                        ...
                        : elementnew(substitution(GridRow.getCellElement(row, example), comparator), true);
        ...
        var splitCellIntoColumns = function(grid, exampleRow, exampleCol, comparator, substitution) {
            ...
            return map(grid,
                function(row, i) {
                    ...
                    var sub = isTargetCell
                        ? elementnew(substitution(GridRow.getCellElement(row, exampleCol), comparator), true)
        ...
        var splitCellIntoRows = function(grid, exampleRow, exampleCol, comparator, substitution) {
            ...
            var before = grid.slice(0, index);
            var after = grid.slice(index);
            var between = GridRow.mapCells(grid[exampleRow],
                function(ex, i) {
        ...
        var notStartRow = function(grid, rowIndex, colIndex, comparator) {
            return GridRow.getCellElement(grid[rowIndex], colIndex) !== undefined &&
            (rowIndex > 0 &&
                comparator(GridRow.getCellElement(grid[rowIndex - 1], colIndex),
                    GridRow.getCellElement(grid[rowIndex], colIndex)));
        };
        var notStartColumn = function(row, index, comparator) {
            return index > 0 && comparator(GridRow.getCellElement(row, index - 1), GridRow.getCellElement(row, index));
        };
        var replaceColumn = function(grid, index, comparator, substitution) {
            var targets = bind(grid,
                function(row, i) {
                    var alreadyAdded = notStartRow(grid, i, index, comparator) ||
        ...
        var replaceRow = function(grid, index, comparator, substitution) {
            ...
            var targets = bind(targetRow.cells(),
                function(item, i) {
                    var alreadyAdded = notStartRow(grid, index, i, comparator) ||
        ...
        var getWidthFrom = function(warehouse, direction, getWidth, fallback, tableSize) {
            ...
            var backups = map(columns,
                function(cellOption) {
                    return cellOption.map(direction.edge);
                });
            return map(columns,
                function(cellOption, c) {
        ...
        var getPercentageWidths = function(warehouse, direction, tableSize) {
            return getWidthFrom(warehouse,
                direction,
                Sizes.getPercentageWidth,
                function(deduced) {
        ...
        var getPixelHeights = function(warehouse, direction) {
            return getHeightFrom(warehouse,
                direction,
                Sizes.getHeight,
                function(deduced) {
        ...
        var base = function(handleUnsupported, required) {
            return baseWith(handleUnsupported,
                required,
                {
                    validate: isFunction,
                    label: "function"
        ...
        var baseWith = function(handleUnsupported, required, pred) {
            ...
            return function(obj) {
                ...
                var invalidKeys = filter(required,
                    function(key) {
                ...
                return last$1(cell).bind(function(rightEdge) {
                    ...
                    return parent(rightEdge).map(function(parent) {
                        return rightSiblingIsBlock === true ||
                            isListItem(parent) ||
                            isBr(rightEdge) ||
                            DomStructure.isBlock(parent) && !eq(cell, parent)
                            ? []
                            : [Element.fromTag("br")];
                    });
                }).getOr([]);
        ...
        var uniqueRows = function(details) {
            return foldl(details,
                function(rest, detail) {
                    return exists(rest,
        ...
        var insertRowBefore = function(grid, detail, comparator, genWrappers) {
            ...
            var newGrid =
                ModificationOperations.insertRowAt(grid, targetIndex, example, comparator, genWrappers.getOrInit);
        ...
        var insertRowsBefore = function(grid, details, comparator, genWrappers) {
            ...
            var newGrid = foldl(rows,
                function(newG, _row) {
                    return ModificationOperations.insertRowAt(newG,
                        targetIndex,
                        example,
                        comparator,
        ...
        var insertColumnBefore = function(grid, detail, comparator, genWrappers) {
            ...
            var newGrid =
                ModificationOperations.insertColumnAt(grid, targetIndex, example, comparator, genWrappers.getOrInit);
        ...
        var insertColumnsBefore = function(grid, details, comparator, genWrappers) {
            ...
            var newGrid = foldl(columns,
                function(newG, _row) {
                    return ModificationOperations.insertColumnAt(newG,
                        targetIndex,
                        example,
                        comparator,
                        genWrappers.getOrInit);
        ...
        var makeColumnHeader = function(grid, detail, comparator, genWrappers) {
            var newGrid =
                TransformOperations.replaceColumn(grid, detail.column(), comparator, genWrappers.replaceOrInit);
            return bundle(newGrid, detail.row(), detail.column());
        };
        ...
        var splitCellIntoColumns$1 = function(grid, detail, comparator, genWrappers) {
            var newGrid = ModificationOperations.splitCellIntoColumns(grid,
                detail.row(),
                detail.column(),
                comparator,
                genWrappers.getOrInit);
        ...
        var splitCellIntoRows$1 = function(grid, detail, comparator, genWrappers) {
            var newGrid = ModificationOperations.splitCellIntoRows(grid,
                detail.row(),
                detail.column(),
                comparator,
                genWrappers.getOrInit);
        ...
        var eraseColumns = function(grid, details, _comparator, _genWrappers) {
            var columns = uniqueColumns(details);
            var newGrid =
                ModificationOperations.deleteColumnsAt(grid, columns[0].column(), columns[columns.length - 1].column());
        ...
        var pasteRowsAfter = function(grid, pasteDetails, comparator, _genWrappers) {
            var example = grid[pasteDetails.cells[0].row()];
            var index = pasteDetails.cells[pasteDetails.cells.length - 1].row() +
        ...
        var TableActions = function(editor, lazyWire) {
            ...
            var execute = function(operation, guard, mutate, lazyWire) {
                return function(table, target) {
                    ...
                    return guard(table)
                        ? operation(wire, table, target, generators, direction).bind(function(result) {
                            each(result.newRows(),
                                function(row) {
                                    fireNewRow(editor, row.dom());
                                });
                            each(result.newCells(),
                                function(cell) {
            ...
            var insertColumnsBefore =
                execute(TableOperations.insertColumnsBefore, always, CellMutations.halve, lazyWire);
        ...
        var copyRows = function(table, target, generators) {
            ...
            return details.map(function(selectedCells) {
                ...
                var slicedGrid = grid.slice(selectedCells[0].row(),
        ...
        var unApplyAlign = function(editor, elm) {
            global$1.each("left center right".split(" "),
                function(name) {
                    editor.formatter.remove("align" + name, {}, elm);
                });
        };
        var unApplyVAlign = function(editor, elm) {
            global$1.each("top middle bottom".split(" "),
                function(name) {
        ...
        var createStyleForm = function(editor) {
            var createColorPickAction = function() {
                ...
                if (colorPickerCallback) {
                    return function(evt) {
                        return colorPickerCallback.call(editor,
                            function(value) {
        ...
        var open = function(editor) {
            ...
            if (getCellClassList(editor).length > 0) {
                classListCtrl = {
                    ...
                    type: "listbox",
                    label: "Class",
                    values: Helpers.buildListItems(getCellClassList(editor),
                        function(item) {
                            if (item.value) {
        ...
        var open$1 = function(editor) {
            ...
            global$1.each(tableElm.rows,
                function(row) {
                    global$1.each(row.cells,
                        function(cell) {
            ...
            if (getRowClassList(editor).length > 0) {
                classListCtrl = {
                    ...
                    type: "listbox",
                    label: "Class",
                    values: Helpers.buildListItems(getRowClassList(editor),
                        function(item) {
                            if (item.value) {
                                item.textStyle = function() {
                                    return editor.formatter.getCssText({
        ...
        var fireEvents = function(editor, table) {
            each(descendants$1(table, "tr"),
                function(row) {
                    fireNewRow(editor, row.dom());
                    each(descendants$1(row, "th,td"),
                        function(cell) {
                            fireNewCell(editor, cell.dom());
                        });
                });
        ...
        var extractDataFromElement$2 = function(editor, tableElm) {
            ...
            var data = {
                ...
                cellpadding: dom.getAttrib(tableElm, "data-mce-cell-padding") ||
                    dom.getAttrib(tableElm, "cellpadding") ||
                    Styles$1.getTDTHOverallStyle(editor.dom, tableElm, "padding"),
                border: dom.getAttrib(tableElm, "data-mce-border") ||
                    dom.getAttrib(tableElm, "border") ||
                    Styles$1.getTDTHOverallStyle(editor.dom, tableElm, "border"),
        ...
        var open$2 = function(editor, isProps) {
            ...
            if (getTableClassList(editor).length > 0) {
                ...
                classListCtrl = {
                    ...
                    type: "listbox",
                    label: "Class",
                    values: Helpers.buildListItems(getTableClassList(editor),
                        function(item) {
                            if (item.value) {
                                item.textStyle = function() {
                                    return editor.formatter.getCssText({
        ...
        var registerCommands = function(editor, actions, cellSelection, selections, clipboardRows) {
            ...
            var getSelectionStartFromSelector = function(selector) {
                return function() {
                    return Option.from(editor.dom.getParent(editor.selection.getStart(), selector))
                        .map(Element.fromDom);
                };
            };
        ...
        var BarManager = function(wire, direction, hdirection) {
            ...
            var mousedown = bind$2(wire.parent(),
                "mousedown",
                function(event) {
            ...
            var mouseover = bind$2(wire.view(),
                "mouseover",
                function(event) {
        ...
        var get$8 = function(editor, container) {
            return editor.inline
                ? ResizeWire.body(getBody$1(editor), createContainer())
                ...
                function(e) {
                    ...
                    if (isTable(targetElm)) {
                        ...
                        if (percentageBasedSizeRegex.test(startRawW)) {
                            ...
                            global$1.each(newCellSizes_1,
                                function(newCellSize) {
        ...
        var fromRange = function(win, type, range) {
            return type(Element.fromDom(range.startContainer),
                range.startOffset,
        ...
        var doDiagnose = function(win, ranges) {
            ...
            if (rng.collapsed) {
                ...
                return reversed.map(function(rev) {
                    return adt$4.rtl(Element.fromDom(rev.endContainer),
                        rev.endOffset,
        ...
        var availableSearch = document.caretPositionFromPoint
        ...
        var fromPoint$1 = function(win, x, y) {
            ...
            return availableSearch(doc, x, y).map(function(rng) {
                return SimRange.create(Element.fromDom(rng.startContainer),
                    rng.startOffset,
        ...
        var readRange = function(selection) {
            if (selection.rangeCount > 0) {
                ...
                return Option.some(SimRange.create(Element.fromDom(firstRng.startContainer),
        ...
        var doGetExact = function(selection) {
            ...
            return after$3(anchor, selection.anchorOffset, focus, selection.focusOffset)
                ? Option.some(SimRange.create(anchor, selection.anchorOffset, focus, selection.focusOffset))
                : readRange(selection);
        };
            ...
            if (!eq(start, finish)) {
                return CellSelection.identify(start, finish, isRoot).bind(function(cellSel) {
                    ...
                    if (boxes.length > 0) {
                        ...
                        return Option.some(Response.create(Option.some(Util.makeSitus(start, 0, start, getEnd(start))),
                            true));
                    } else {
                        return Option.none();
                    }
        ...
        var update = function(rows, columns, container, selected, annotations) {
            ...
            return CellSelection.shiftSelection(selected,
                rows,
                columns,
                annotations.firstSelectedSelector(),
                annotations.lastSelectedSelector()).map(updateSelection);
        };
        var KeySelection = {
            sync: sync,
            detect: detect$5,
            ...
            return closest$1(after, "td,th", isRoot).bind(function(afterCell) {
                return closest$1(before, "td,th", isRoot).map(function(beforeCell) {
                    if (!eq(afterCell, beforeCell)) {
                        ...
                        return eq(after, afterCell) && getEnd(afterCell) === afterOffset
                            ? failure(beforeCell)
                            : adt$5.none("in same cell");
                    }
                });
        ...
        var gatherer = function(cand, gather, isRoot) {
            return gather(cand, isRoot).bind(function(target) {
                return isText(target) && get$2(target).trim().length === 0
        ...
        var inOutsideBlock = function(bridge, element, caret) {
            return closest(element, DomStructure.isBlock).fold(constant(false),
                function(cell) {
        ...
        var adjustDown = function(bridge, element, guessBox, original, caret) {
            ...
            if (Math.abs(guessBox.bottom() - original.bottom()) < 1) {
                ...
                return inOutsideBlock(bridge, element, caret)
                    ? adt$6.retry(Carets.translate(lowerCaret, JUMP_SIZE, 0))
                    : adt$6.none();
            }
        };
        var adjustUp = function(bridge, element, guessBox, original, caret) {
            ...
            if (Math.abs(guessBox.top() - original.top()) < 1) {
                ...
                return inOutsideBlock(bridge, element, caret)
                    ? adt$6.retry(Carets.translate(higherCaret, JUMP_SIZE, 0))
                    : adt$6.none();
            }
        };
        var upMovement = {
        ...
        var adjustTil = function(bridge, movement, original, caret, numRetries) {
            ...
            return bridge.situsFromPoint(caret.left(), movement.point(caret)).bind(function(guess) {
                return guess.start().fold(Option.none,
                    function(element) {
                        return Rectangles.getEntireBox(bridge, element).bind(function(guessBox) {
                            return movement.adjuster(bridge, element, guessBox, original, caret).fold(Option.none,
                                function(newCaret) {
            ...
            return bridge.getSelection().bind(function(sel) {
                return BrTags.tryBr(isRoot, sel.finish(), sel.foffset(), direction).fold(function() {
                    ...
                    function(brNeighbour) {
                        ...
                        var analysis = BeforeAfter.verify(bridge,
                            sel.finish(),
                            sel.foffset(),
                            range.finish(),
                            range.foffset(),
        ...
        var tryAt = function(bridge, direction, box) {
            if (platform$1.browser.isChrome() ||
                platform$1.browser.isSafari() ||
                platform$1.browser.isFirefox() ||
                platform$1.browser.isEdge()) {
            ...
            if (detection.browser.isIE()) {
                ...
                return precheck(initial, isRoot).orThunk(function() {
                    return simulate(bridge, isRoot, direction, initial, anchor).map(function(info) {
                        ...
                        return Response.create(Option.some(Util.makeSitus(range.start(),
                                range.soffset(),
                                range.finish(),
                                range.foffset())),
                            true);
            ...
            return closest$1(initial, "tr", isRoot).bind(function(startRow) {
                return closest$1(startRow, "table", isRoot).bind(function(table) {
                    ...
                    if (eq(startRow, rows[0])) {
                        return seekLeft$1(table,
                            function(element) {
                                return last$1(element).isSome();
                            },
                            isRoot).map(function(last) {
                            var lastOffset = getEnd(last);
                            return Response.create(Option.some(Util.makeSitus(last, lastOffset, last, lastOffset)),
                                true);
                        });
                    } else {
                        return Option.none();
            ...
            var keydown = function(event, start, soffset, finish, foffset, direction) {
                ...
                var handler = CellSelection.retrieve(container, annotations.selectedSelector()).fold(function() {
                        if (SelectionKeys.isDown(keycode) && shiftKey) {
                            return curry(VerticalMovement.select,
                                bridge,
                                container,
                                isRoot,
                                KeyDirection.down,
                            ...
                            return curry(VerticalMovement.navigate,
                                bridge,
                                isRoot,
                                KeyDirection.down,
                                finish,
                    ...
                    function(selected) {
                        var update = function(attempts) {
                            return function() {
                                var navigation = findMap(attempts,
                                    function(delta) {
                                        return KeySelection.update(delta.rows(),
                                            delta.cols(),
                                            container,
                                            selected,
                                            annotations);
                                ...
                                return navigation.fold(function() {
                                        return CellSelection
                                            .getEdges(container,
                                                annotations.firstSelectedSelector(),
                                                annotations.lastSelectedSelector()).map(function(edges) {
                                                var relative = SelectionKeys.isDown(keycode) ||
                                                    direction.isForward(keycode)
                                                    ? Situ.after
                                                    : Situ.before;
                                                bridge.setRelativeSelection(Situ.on(edges.first(), 0),
            ...
            var keyup = function(event, start, soffset, finish, foffset) {
                return CellSelection.retrieve(container, annotations.selectedSelector()).fold(function() {
                        ...
                        if (SelectionKeys.isNavigation(keycode)) {
                            return KeySelection.sync(container,
                                isRoot,
                                start,
                                soffset,
                                finish,
            ...
            editor.on("init",
                function(e) {
                    ...
                    var keydown = function(event) {
                        ...
                        var start = Element.fromDom(rng.startContainer);
                        var end = Element.fromDom(rng.endContainer);
                        var direction = Direction.directionAt(startContainer).isRtl()
                            ? SelectionKeys.rtl
                            : SelectionKeys.ltr;
        ...
        var addButtons = function(editor) {
            var menuItems = [];
            each$4("inserttable tableprops deletetable | cell row column".split(" "),
                function(name) {
                    if (name === "|") {
                        menuItems.push({ text: "-" });
                    } else {
        ...
        var addMenuItems = function(editor, selections) {
            ...
            var generateTableGrid = function() {
                ...
                for (var y = 0; y < 10; y++) {
                    html += "<tr>";
                    for (var x = 0; x < 10; x++) {
                        html += '<td role="gridcell" tabindex="-1"><a id="mcegrid' +
                            (y * 10 + x) +
                            '" href="#" ' +
                            'data-mce-x="' +
                            x +
