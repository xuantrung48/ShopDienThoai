...
(function(global, factory) {
    ...
    (function(exports) {
        ...
        function report(_ref) {
            ...
            if (conflictsCount > 0) {
                console.info("%cConflict".concat(conflictsCount > 1 ? "s" : "", " found:"),
            ...
            if (noConflictsCount > 0) {
                console.info(
                    "%cNo conflict".concat(noConflictsCount > 1 ? "s" : "", " found with ")
            ...
            function runDiag(scriptOrLinkTag, md5) {
                var diagFrame =
                    DOCUMENT.createElement(
                        "iframe"); // Using "visibility: hidden; position: absolute" instead of "display: none;" because
                // Firefox will not return the expected results for getComputedStyle if our iframe has display: none.

                diagFrame.setAttribute("style", "height: 0; position: absolute; visibility: hidden; width: 0;");
                var testIconId = "fa-test-icon-" + md5;
                var iTag = DOCUMENT.createElement("i");
                iTag.setAttribute("class", "fa fa-coffee");
                iTag.setAttribute("id", testIconId);
                var diagScript = DOCUMENT.createElement("script");
                diagScript.setAttribute("id",
                    diagScriptId); // WARNING: this function will be toString()'d and assigned to innerText of the diag script
                ...
                diagScript.innerText = "(".concat(diagScriptFun.toString(), ")('").concat(nodeUnderTestId, "', '")
                    .concat(testIconId || "foo", "', '").concat(md5, "', '").concat(parentOrigin, "');");
        ...
        function detectSvgConflicts(currentScript) {
            ...
            var _loop = function _loop(scriptIdx) {
                ...
                diagScript.innerText = "(".concat(diagScriptFun.toString(), ")('").concat(nodeUnderTestId, "', '")
                    .concat(md5ForScript, "', '").concat(parentOrigin, "');");
        ...
        function conflictDetection() {
            ...
            var testCount =
                Object.keys(scriptsToTest).length +
                    Object.keys(cssToTest)
                    .length; // The resultsCollectionMaxWait allows for the time between when the tests running under
            ...
            if (testCount === 0) {
                ...
                console.info(
                    "We'll wait about ".concat(Math.round(WINDOW.FontAwesomeDetection.timeout / 10) / 100,
                ...
                pollUntil({
                    ...
                    fn: function fn() {
                        return Object.keys(nodesTested.conflict).length + Object.keys(nodesTested.noConflict).length >=
                            testCount;
                    }
                }).then(function() {
                    console.info("DONE!");
