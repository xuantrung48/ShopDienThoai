...
(function() {
    ...
    var RESERVED_CLASSES = [
        "xs", "sm", "lg", "fw", "ul", "li", "border", "pull-left", "pull-right", "spin", "pulse", "rotate-90",
        "rotate-180", "rotate-270", "flip-horizontal", "flip-vertical", "flip-both", "stack", "stack-1x", "stack-2x",
        "inverse", "layers", "layers-text", "layers-counter", DUOTONE_CLASSES.GROUP, DUOTONE_CLASSES.SWAP_OPACITY,
        DUOTONE_CLASSES.PRIMARY, DUOTONE_CLASSES.SECONDARY
    ...
    if (DOCUMENT && typeof DOCUMENT.querySelector === "function") {
        var attrs = [
            ["data-family-prefix", "familyPrefix"], ["data-replacement-class", "replacementClass"],
            ["data-auto-replace-svg", "autoReplaceSvg"], ["data-auto-add-css", "autoAddCss"],
            ...
            ["data-keep-original-source", "keepOriginalSource"], ["data-measure-performance", "measurePerformance"],
    ...
    function transformIsMeaningful(transform) {
        return transform.size !== meaninglessTransform.size ||
    ...
    function asIcon(_ref) {
        ...
        if (transformIsMeaningful(transform) && main.found && !mask.found) {
            ...
            attributes["style"] = joinStyles(_objectSpread({},
                styles,
                {
                    'transform-origin': "".concat(offset.x + transform.x / 16, "em ")
    ...
    function makeInlineSvgAbstract(params) {
        ...
        var attrClass =
            [config.replacementClass, iconName ? "".concat(config.familyPrefix, "-").concat(iconName) : "", widthClass]
                .filter(function(c) {
    ...
    var end = function end(name) {
        ...
        p.measure("".concat(preamble, " ").concat(name),
    ...
    function toHtml(abstractNodes) {
        ...
        if (typeof abstractNodes === "string") {
            ...
            return "<".concat(tag, " ").concat(joinAttributes(attributes), ">")
                .concat(children.map(toHtml).join(""), "</").concat(tag, ">");
        }
    }
    ...
    var OPACITY_ANIMATE = _objectSpread({},
        ANIMATION_BASE,
        {
    ...
    function findIcon(iconName, prefix) {
        return new picked(function(resolve, reject) {
            ...
            if (_typeof(WINDOW.FontAwesomeKitConfig) === "object" &&
                typeof window.FontAwesomeKitConfig.token === "string") {
                headers["fa-kit-token"] = WINDOW.FontAwesomeKitConfig.token;
            }

            if (iconName && prefix && !config.showMissingIcons) {
                reject(new MissingIcon(
    ...
    function onTree(root) {
        ...
        var prefixesDomQuery = [".".concat(LAYERS_TEXT_CLASSNAME, ":not([").concat(DATA_FA_I2SVG, "])")].concat(
    ...
    function replaceForPosition(node, position) {
        ...
        return new picked(function(resolve, reject) {
            ...
            if (alreadyProcessedPseudoElement && !fontFamily) {
                ...
                var prefix = ~["Solid", "Regular", "Light", "Duotone", "Brands"].indexOf(fontFamily[1])
                    ? STYLE_TO_PREFIX[fontFamily[1].toLowerCase()]
                    : FONT_WEIGHT_TO_PREFIX[fontWeight];
                var hexValue = toHex(content.length === 3 ? content.substr(1, 1) : content);
                var iconName = byUnicode(prefix, hexValue);
                var iconIdentifier =
                    iconName; // Only convert the pseudo element in this :before/:after position into an icon if we haven't
                ...
                (!alreadyProcessedPseudoElement ||
                    ...
                    findIcon(iconName, prefix).then(function(main) {
                        var abstract = makeInlineSvgAbstract(_objectSpread({},
                            meta,
                            {
                                icons: {
                                    main: main,
    ...
    function css() {
        ...
        if (fp !== dfp || rc !== drc) {
            ...
            s = s.replace(dPatt, ".".concat(fp, "-")).replace(customPropPatt, "--".concat(fp, "-"))
                ...
                [
                    {
                        ...
                        value: function add() {
                            ...
                            for (var _len = arguments.length, definitions = new Array(_len), _key = 0;
                                _key < _len;
                                _key++) {
                            ...
                            Object.keys(additions).forEach(function(key) {
                                _this.definitions[key] =
                                    _objectSpread({}, _this.definitions[key] || {}, additions[key]);
                ...
                return [
                    {
                        tag: "span",
                        attributes: {
                            class: ["".concat(config.familyPrefix, "-layers")].concat(_toConsumableArray(classes))
                                .join(" ")
                        },
                        children: children
                    }
